...
const OwnersModal: FC = () => {
    ...
    const handleChange = (id) => {
        ...
        const filterIndex = newPickedFilters?.findIndex((item) => item?.filter?.attributeKey === 'DataOwners');
        const newOperator = intersectionLogic === 'or' ? 'contains' : 'matchAll';
        if (filterIndex === -1) {
            const filter = {
                icon: usersFields.find((item) => item.id === id).icon,
                filter: { attributeKey: 'DataOwners', operator: newOperator, values: [id] },
            };
    ...
    return (
        <ModalBase
            ...
            isOpen={DataOwners}
            label={chrome.i18n.getMessage(`attribute_key_DataOwners`)}
